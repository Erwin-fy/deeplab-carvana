...
if is_server
  ...
  elseif strcmp(dataset, 'coco')
   ...
   elseif strcmp(dataset, 'cxr')
    %VOC_root_folder = '/media/secondDisk/huangyinya/deeplab-public-ver2/data/cxr/test170731';
    VOC_root_folder = '~/deeplab-public-ver2/data/cxr';
   elseif strcmp(dataset, 'carvana')
    VOC_root_folder = '/media/Disk/wangfuyu/Carvana/test';
  else
    error('Wrong dataset');
  end
end
...
output_mat_folder = fullfile('~/deeplab-public-ver2/exper', dataset, feature_name, model_name, testset, feature_type);

save_root_folder = fullfile('~/deeplab-public-ver2/exper', dataset, 'res', feature_name, model_name, testset, feature_type, post_folder);
...
elseif strcmp(dataset, 'cxr')
  seg_res_dir = [save_root_folder '/results/CXRSet/'];
  seg_root = fullfile(VOC_root_folder, '');
  gt_dir   = fullfile(VOC_root_folder, '', 'SegmentationClass');
elseif strcmp(dataset, 'carvana')
  seg_res_dir = [save_root_folder '/results/Carvana/'];
  seg_root = fullfile(VOC_root_folder, '');
  gt_dir   = fullfile(VOC_root_folder, '', 'SegmentationClass');
...
elseif strcmp(dataset, 'cxr')
  VOCopts = GetVOCopts(seg_root, seg_res_dir, trainset, testset, 'CXRSet');
elseif strcmp(dataset, 'carvana')
  VOCopts = GetVOCopts(seg_root, seg_res_dir, trainset, testset, 'Carvana');
end

if is_mat
  % crop the results
  load('pascal_seg_colormap.mat');
  display(output_mat_folder);
  output_dir = dir(fullfile(output_mat_folder, '*.mat'));
  display(numel(output_dir));
  for i = 1 : numel(output_dir)
    ...
    elseif strcmp(dataset, 'cxr')
      img = imread(fullfile(VOC_root_folder, 'JPEGImages', [img_fn, '.jpg'])); 
    elseif strcmp(dataset, 'carvana')
      img = imread(fullfile(VOC_root_folder, 'JPEGImages', '0', [img_fn, '.jpg'])); 
    end
    ...
    %display(fullfile(save_result_folder, [img_fn, '.png']));
  end
end

% get iou score
if strcmp(testset, 'testJM')
